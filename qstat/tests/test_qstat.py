...
qstat_example_xml_path = pkg_resources.resource_filename(
    "qstat", os.path.join("tests", "resources", "qstat_example.xml")
)

qstat_xml_empty_queue_path = pkg_resources.resource_filename(
    "qstat", os.path.join("tests", "resources", "qstat_empty_queue.xml")
)

qstat_length_one_xml_path = pkg_resources.resource_filename(
    "qstat", os.path.join("tests", "resources", "qstat_length_one.xml")
)
...
def test_read_empty_qstat():
    ...
    queue_info, job_info = qstat._tools.xml2queue_and_job_info(
        qstat_xml_empty_queue
    )
...
def test_qsat_not_installed_exception():
    with tempfile.TemporaryDirectory(prefix="qstat_") as tmp:
        ...
        try:
            qstat._tools.qstat2xml(qstat_path=nap)
...
def test_qsat_xml_oprion():
    with tempfile.TemporaryDirectory(prefix="qstat_") as tmp:
        ...
        try:
            qstat._tools.qstat2xml(qstat_path=nap, xml_option=xml_option)
        except FileNotFoundError as e:
            assert (
                'Maybe "' + nap + " " + xml_option + '" is not installed.'
                in e.message
            )
